	...
	logsAndEvents := map[string]coreData.LogHandler{
			...
			Events: []*transaction.Event{
				{
					...
					Topics:     [][]byte{[]byte("mect"), big.NewInt(0).Bytes(), big.NewInt(0).SetUint64(100).Bytes(), []byte("receiver")},
				},
				nil,
			},
		},
		"h4": &transaction.Log{
			Events: []*transaction.Event{
				{
					Address:    []byte("addr"),
					Identifier: []byte(issueSemiFungibleMECTFunc),
					Topics:     [][]byte{[]byte("SEMI-abcd"), []byte("semi-token"), []byte("SEMI"), []byte(core.SemiFungibleMECT)},
				},
				nil,
			},
		},
