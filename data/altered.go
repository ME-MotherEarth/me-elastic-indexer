...
func (aa *alteredAccounts) Add(key string, account *AlteredAccount) {
	...
	isTokenOperation := account.IsMECTOperation || account.IsNFTOperation
	if !isTokenOperation {
	...
	for _, elem := range aa.altered[key] {
		newElementIsTokenOperation := account.IsMECTOperation || account.IsNFTOperation
		oldElementIsTokenOperation := elem.IsMECTOperation || elem.IsNFTOperation
		...
		if shouldRewrite {
			...
			elem.IsMECTOperation = account.IsMECTOperation
