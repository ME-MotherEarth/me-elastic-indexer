...
type TokensHandler interface {
	...
	PutTypeAndOwnerInAccountsMECT(accountsMECT map[string]*AccountInfo)
	GetAllTokens() []string
	GetAll() []*TokenInfo
	GetAllWithoutMetaMECT() []*TokenInfo
	IsInterfaceNil() bool
}
...
// GetAllWithoutMetaMECT will return all tokens except metaMECT tokens
func (ti *tokensInfo) GetAllWithoutMetaMECT() []*TokenInfo {
	tokens := make([]*TokenInfo, 0)
	for _, tokenData := range ti.tokensInfo {
		if tokenData.Type == metaMECT {
			continue
		}
...
// PutTypeAndOwnerInAccountsMECT will put in the provided accounts MECT map token type and current owner
func (ti *tokensInfo) PutTypeAndOwnerInAccountsMECT(accountsMECT map[string]*AccountInfo) {
	for _, accountMECT := range accountsMECT {
		tokenData, ok := ti.tokensInfo[accountMECT.TokenIdentifier]
		if !ok {
			continue
		}

		accountMECT.Type = tokenData.Type
		accountMECT.CurrentOwner = tokenData.CurrentOwner
	}
}
